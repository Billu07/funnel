...
import { validateTrialLeadSubmission } from "@/lib/funnel-validation";
...
function getWebhookUrl(): string {
  return (
    process.env.N8N_WEBHOOK_TRIAL_URL ??
...
export async function POST(request: Request) {
  ...
  const validation = validateTrialLeadSubmission(payload);
  if (!validation.ok) {
    return NextResponse.json(
  ...
  try {
    ...
    if (!webhookResponse.ok) {
      console.error("Trial webhook failed", {
    ...
    console.error("Trial webhook request error", error);
    return NextResponse.json(
      { error: "Could not process your request." },
      { status: 502 },
    );
